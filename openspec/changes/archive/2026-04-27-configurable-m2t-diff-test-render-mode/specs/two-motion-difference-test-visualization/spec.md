...
#### Scenario: Configured slow rendering applies to all exported videos
- **WHEN** a MotionFix `m2t_diff` test run enables visualization and sets the visualization render method to `slow`
- **THEN** the exported source, target, and side-by-side videos are all rendered with the slow visualization pipeline for that sample
...
- **WHEN** an exported `m2t_diff` test sample refers to a MotionFix pair whose cached source or target render already exists in the MotionFix render cache for the configured render method
...
- **WHEN** an exported `m2t_diff` test sample refers to a MotionFix pair whose cached source-target comparison render already exists for the configured render method
- **THEN** the test visualization path reuses that cached side-by-side artifact for the sample output

#### Scenario: Fast and slow caches remain isolated
- **WHEN** one MotionFix `m2t_diff` test run renders a sample with `fast` mode and another renders the same sample with `slow` mode
- **THEN** each render mode uses a different cache location so neither mode overwrites or incorrectly reuses the other mode's cached artifact
