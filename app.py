...
def load_motion(motion_uploaded, method):
    ...
    output_mp4_path, video_fname, output_npy_path, joints_fname = render_motion(
        joints, feats.to("cpu").numpy(), method
    )
...
def add_audio(history, audio_path, data_stored, language="en"):
    ...
    text_input = audio_processor.batch_decode(predicted_ids, skip_special_tokens=True)
...
def bot(history, motion_uploaded, data_stored, method, task="t2m"):
    ...
    prompt = model.lm.placeholder_fulfill(
        input, motion_length, model.lm.input_motion_holder_seq, ""
    )
    ...
    batch = {
        ...
        "text": [prompt],
        "motion_tokens_input": [motion_tokens_input]
        if motion_tokens_input is not None
        else None,
    ...
    if task in ["t2t", "m2t"]:
        ...
        out_texts = outputs["texts"][0]
        output_mp4_path, video_fname, output_npy_path, joints_fname = render_motion(
            out_joints, out_feats.to("cpu").numpy(), method
        )
    ...
    if "<Motion_Placeholder>" == out_texts:
        ...
        response = [
            Text_Components.format(msg=out_texts.split("<Motion_Placeholder>")[0]),
            Video_Components.format(
                video_path=output_mp4_path,
                video_fname=video_fname,
                motion_path=output_npy_path,
            ...
            Text_Components.format(msg=out_texts.split("<Motion_Placeholder>")[1]),
        ]
    else:
...
with gr.Blocks(css=customCSS) as demo:
    ...
    m2t_examples = gr.State(
        [
            ...
            (
                Video_Components_example.format(
                    video_path="assets/videos/0_out.mp4", video_fname="example0.mp4"
                ),
            ...
            (
                Video_Components_example.format(
                    video_path="assets/videos/m2t/1_out.mp4", video_fname="example4.mp4"
                ),
    ...
    with gr.Row():
        with gr.Column(scale=0.85):
            ...
            with gr.Row():
                aud = gr.Audio(
                    sources="microphone", label="Speak input", type="filepath"
                )
                btn = gr.UploadButton(
                    "📁 Upload motion", elem_id="upload", file_types=["file"]
                )
    ...
    txt_msg = txt.submit(
        ...
        bot,
        [chatbot, motion_uploaded, data_stored, method, task],
    ...
    file_msg = btn.upload(
        add_file,
        [chatbot, btn, txt, motion_uploaded],
        [chatbot, txt, motion_uploaded],
        queue=False,
    )
    aud_msg = aud.stop_recording(
        add_audio,
        [chatbot, aud, data_stored, language],
        [chatbot, data_stored],
        queue=False,
    ).then(
        bot,
    ...
    t2m_eg_msg = t2m_eg.click(
        bot_example, [chatbot, t2m_examples], [chatbot], queue=False
    )
    m2t_eg_msg = m2t_eg.click(
        bot_example, [chatbot, m2t_examples], [chatbot], queue=False
    )
    t2t_eg_msg = t2t_eg.click(
        bot_example, [chatbot, t2t_examples], [chatbot], queue=False
    )
