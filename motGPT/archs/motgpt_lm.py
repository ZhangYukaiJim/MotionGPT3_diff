...
class MotionUndHead(nn.Module):
    def __init__(
        self, input_dim, output_dim, projector_type="linear", depth=1, **kwargs
    ):
        super().__init__()
...
class MLM(nn.Module):
        ...
        **kwargs,
        ...
        if motion_holder_seq_mode == "withse":
            ...
            self.output_motion_holder_seq = (
                "<start_of_motion>"
                + self.motion_holder_output_word * self.motion_holder_repeat
                + "<end_of_motion>"
            )
            self.input_motion_holder_seq = (
                "<start_of_motion>"
                + self.motion_holder_word * self.holder_num_in_input
                + "<end_of_motion>"
            )
        ...
        if model_type == "t5":
            ...
            language_model = MoTT5ForConditionalGeneration(
            ...
            language_model = MoTGPT2LMHeadModel(
                mconfig,
                motion_codebook_size=all_motion_token_num,
                mot_factor=mot_factor,
        ...
        self.tokenizer.add_special_tokens(
            {"additional_special_tokens": new_special_tokens_text}
        )
        self.text_vocab_size = len(self.tokenizer)

        self.tokenizer.add_special_tokens(
            {"additional_special_tokens": new_special_tokens_mod}
        )
        self.tokenizer.add_tokens(new_tokens)
        self.som_id, self.eom_id = self.tokenizer.convert_tokens_to_ids(
            [self.som_word, self.eom_word]
        )
        # self.mom_id, self.pom_id =  self.tokenizer.convert_tokens_to_ids([self.mom_word, self.pom_word])
        self.motion_holder_id_out, self.motion_holder_id = (
            self.tokenizer.convert_tokens_to_ids(
                [self.motion_holder_output_word, self.motion_holder_word]
            )
        )
        ...
        self.language_model.config.mot_lm_dim = (
            self.language_model.config.motion_vocab_size
        )
        ...
        self.motion_und_head = MotionUndHead(
            vae_latent_channels,
            self.llm_decoder_embed_dim * self.holder_num_in_input,
        ...
        if self.multi_hidden:
            ...
            if self.with_hid_norm:
                ...
                self.diffusion_pos_embed_learned = nn.Parameter(
                    torch.zeros(
                        1, self.motion_holder_repeat, self.llm_decoder_embed_dim
                    )
                )
        ...
        if self.do_classifier_free_guidance:
            if fake_latent_mode == "learnable_zero":
                self.fake_latent = nn.Parameter(
                    torch.zeros(self.motion_holder_repeat, self.llm_decoder_embed_dim)
                ...
                self.fake_latent = torch.zeros(
                    self.motion_holder_repeat, self.llm_decoder_embed_dim
                )
            else:
                assert False, (
                    f"not Implemented fake_latent_mode {fake_latent_mode}, should in \[learnable_zero, all_zero, learnable_rand]"
                )
    ...
    def build_two_motion_prompt(self, prompt: str, length1: int, length2: int):
        seconds1 = math.floor(length1 / self.framerate)
        seconds2 = math.floor(length2 / self.framerate)
        return (
            prompt.replace("<Motion_Placeholder_s1>", self.input_motion_holder_seq)
            .replace("<Motion_Placeholder_s2>", self.input_motion_holder_seq)
            .replace("<Frame_Placeholder_s1>", f"{length1}")
            .replace("<Frame_Placeholder_s2>", f"{length2}")
            .replace("<Second_Placeholder_s1>", f"{seconds1:.1f}")
            .replace("<Second_Placeholder_s2>", f"{seconds2:.1f}")
        )

    def encode_two_motion_tokens(
        self,
        motion_encode_net,
        motion_source: Tensor,
        motion_target: Tensor,
        lengths_source: List[int],
        lengths_target: List[int],
    ):
        source_tokens = self.motion_feats_to_tokens(
            motion_encode_net, motion_source, lengths_source, modes="motion"
        )
        target_tokens = self.motion_feats_to_tokens(
            motion_encode_net, motion_target, lengths_target, modes="motion"
        )
        combined_tokens = []
        for src, tgt in zip(source_tokens, target_tokens):
            if src is None or tgt is None:
                combined_tokens.append(None)
            else:
                combined_tokens.append(torch.cat([src, tgt], dim=0))
    ...
    def forward_diff_loss(self, z, target):
        ...
        if self.do_classifier_free_guidance:
            z = torch.stack(
                [
                    self.fake_latent if np.random.rand(1) < self.guidance_uncondp else k
                    for k in z
                ]
            )
    ...
    def sample_tokens(
        self, outputs, device, temperature=1.0, cfg=1.0, vae_mean_std_inv=None
    ):
        ...
        if isinstance(outputs, List):
            ...
            for i, output in enumerate(outputs):
                if output is not None:  # [m_num,768]
                    ...
                    motion_mask[i] = 1
            output_to_sample = torch.stack(output_to_sample, dim=0).to(
                device
            )  # [bs,10,768]
        else:
            motion_mask = torch.zeros(len(outputs))
        ...
        if cfg > 1.0:
            output_to_sample = torch.cat(
                [
                    output_to_sample,
                    self.fake_latent[None, :, :].repeat(
                        output_to_sample.shape[0], 1, 1
                    ),
                ],
                dim=0,
            )  # [bs*2,10,768]
        ...
        sampled_token_latents = self.diffloss.sample(
            output_to_sample, temperature, cfg
        )  # [bs,256]
        if cfg > 1.0:
            sampled_token_latents, _ = sampled_token_latents.chunk(
                2, dim=0
    ...
    def forward_encdec(
        ...
        if condition in ["text", "motion"]:
            ...
            source_input_ids = self.filter_input_ids(
                source_input_ids, input_ids_sentinel
            )
        else:
            if "2t" in " ".join(modes):
        ...
        with torch.no_grad():
            motion_tokens = self.motion_feats_to_tokens(
                motion_encode_net, motion_feats, lengths, modes=modes
            )

        # if self.interleaved_input:
        inputs_embeds = self.language_model.get_embeddings_from_ids(
            source_input_ids, type_ids
        )
        ...
        for i in range(batch_size):
            if motion_tokens[i] is None:  # motion_tokens[i]: [motion_num,1,256]
                continue
            motion_embeddings = self.motion_und_head(
                motion_tokens[i]
            )  # [motion_num, 768]
            motion_embs = motion_embeddings[: motion_num_in_input[i]].split(
                self.llm_decoder_embed_dim, dim=-1
            )
            inputs_embeds[self.mod_id][i, input_is_motion[i], :] = torch.cat(
                motion_embs, dim=0
            )
        ...
        try:
            ...
            hidden_to_diff = [
                hidden[i : i + 1, m, :]
                for i, m in enumerate(output_is_motion)
                if m.sum() > 0
            ]
            hidden_to_diff = torch.cat(
                hidden_to_diff, dim=0
            )  # bs, motion_holder_repeat, 768
            if self.with_hid_norm:
            ...
            outputs.diff_loss = torch.tensor(0.0)
    ...
    def forward_dec(
        ...
        for i in range(batch_size):
            if (
                motion_tokens[i] is None
            ):  # or (motion_num_in_input[i]<1): # motion_tokens[i]: [motion_num,1,256]
                continue
        ...
        try:
            ...
            hidden = outputs.hidden_states[-1][
                self.mod_id
            ]  # [:, :-1, :]  # [bs, labels_seq_len, emb_dim(768)]
            hidden_to_diff = [
                hidden[i : i + 1, m, :]
                for i, m in enumerate(output_is_motion)
                if m.sum() > 0
            ...
            if self.with_hid_norm:
                hidden_to_diff = self.norm_layer(hidden_to_diff)
                hidden_to_diff = hidden_to_diff + self.diffusion_pos_embed_learned
            outputs.diff_loss = self.forward_diff_loss(
                z=hidden_to_diff.view(len(ll), -1, self.hidden_dim), target=ll
            )
    ...
    def generate_direct(
        ...
        if motion_tokens is not None:  # motion_num,1,256
            motion_num_in_input = (
                input_is_motion.sum(dim=-1) // self.holder_num_in_input
            )
            ...
            assert (~input_is_motion).all(), (
                "motion holder in input text, wihout provided motion tokens"
            )

        mid = self.mod2id[gen_mode]
        som_id = (
            self.language_model.modality_infos[mid].som_id
            + self.language_model.modality_infos[mid].token_id_start
        )
        eom_id = (
            self.language_model.modality_infos[mid].eom_id
            + self.language_model.modality_infos[mid].token_id_start
        )
        ...
        outputs_string = self.tokenizer.batch_decode(
            outputs1, skip_special_tokens=False
        )
    ...
    def generate_direct_motion(
        ...
        if self.lm_type == "encdec":
            ...
            output_type_ids = torch.zeros_like(labels_input_ids).to(
                inputs_embeds.device
            )
            output_type_ind = labels_input_ids >= self.text_vocab_size
            output_type_ids[output_type_ind] = 1
            ...
            hidden = outputs.decoder_hidden_states[-1][
                self.mod_id
            ]  # bs, tok_max_len, lat_dim(768)
            latents = torch.stack(
                [h[output_is_motion[i], :] for i, h in enumerate(hidden)]
            )  # bs, motion_holder_repeat, lat_dim(768)
        elif self.lm_type == "dec":
    ...
    def generate_conditional(
        ...
        motion_feats_b: Optional[Tensor] = None,
        motion_encode_net=None,
        lengths: Optional[List[int]] = None,
        lengths_b: Optional[List[int]] = None,
        task: str = "t2m",
        with_len: bool = False,
        stage: str = "train",
        tasks: dict = None,
        ...
        if task in ["t2m", "m2m", "pred", "inbetween"]:
            if task == "t2m":
                ...
                with torch.no_grad():
                    motion_tokens = self.motion_feats_to_tokens(
                        motion_encode_net, motion_feats, lengths, modes="pred"
                    )
                ...
                with torch.no_grad():
                    ...
                    motion_tokens = self.motion_feats_to_tokens(
                        motion_encode_net, motion_feats, lengths, modes="inbetween"
                    )
        ...
        elif task in ["m2t", "t2t", "m2t_diff"]:
            if task == "m2t":
                assert motion_feats is not None and lengths is not None

                with torch.no_grad():
                    motion_tokens = self.motion_feats_to_tokens(
                        motion_encode_net, motion_feats, lengths, modes="motion"
                    )
                ...
                assert motion_feats is not None and motion_feats_b is not None
                assert lengths is not None and lengths_b is not None
                motion_tokens = self.encode_two_motion_tokens(
                    motion_encode_net, motion_feats, motion_feats_b, lengths, lengths_b
                )
                tasks = [
                    {
                        "class": "m2t_diff",
    ...
    def motion_feats_to_tokens(
        ...
        for i in range(bs):
            ...
            motion_feats_to_encode = torch.zeros(
                (3, length, feat_dim), requires_grad=False
            )
            if mode in ["motion", "t2m", "m2t", "l2m", "n2m", "m2l"]:
                ...
                motion_feats_to_encode[0, :predict_head, :] = motion_feat[
                    :predict_head, :
                ]
                motion_feats_to_encode[1, : length - predict_head, :] = motion_feat[
                    predict_head:, :
                ]
                ...
                motion_feats_to_encode[0, :masked_head, :] = motion_feat[
                    :masked_head, :
                ]
                motion_feats_to_encode[1, : length - masked_tail, :] = motion_feat[
                    masked_tail:, :
                ]
                ...
                assert False, mode
            motion_feats_to_encode = motion_feats_to_encode[
                : len(lengths_input), :max_len
            ]
            # z, dist = motion_encode_net.encode(motion_feats_to_encode.to(motion_feats.device), lengths_input)

            dist = motion_encode_net.encode_dist(
                motion_feats_to_encode.to(motion_feats.device), lengths_input
            )
    ...
    def clean_text_string(self, motion_string: List[str]):
        ...
        for i in range(len(motion_string)):
            ...
            output_string.append(
                motion_string[i]
                .replace(self.output_motion_holder_seq, "<Motion_Placeholder>")
                .replace(self.input_motion_holder_seq, "<Motion_Placeholder>")
    ...
    def placeholder_fulfill(
        self, prompt: str, length: int, motion_string: str, text: str
    ):
        ...
        prompt = (
            ...
            .replace("<Motion_Placeholder_s1>", motion_string)
            .replace("<Motion_Placeholder_s2>", motion_string)
            .replace("<Motion_Placeholder_Masked>", motion_masked)
        )
        if "Placeholder" in prompt:
            startIndex = prompt.index("Placeholder")
            assert False, (
                "found unsupported PlaceHolder"
                + prompt[startIndex - 8 : startIndex + 20]
            )

        return prompt

    def placeholder_fulfill_pair(
        self, prompt: str, length1: int, length2: int, text: str
    ):
        prompt = self.build_two_motion_prompt(prompt, length1, length2)
        if random.random() < self.quota_ratio and ('"' not in text):
            text = f'"{text}"'
    ...
    def template_fulfill(self, tasks, lengths, texts, stage="test"):
        inputs = []
        outputs = []
        modes = []
        ...
        for i in range(len(lengths)):
            ...
            length = lengths[i]
            inputs.append(
                self.placeholder_fulfill(
                    input_template, length, self.input_motion_holder_seq, texts[i]
                )
            )
            outputs.append(
                self.placeholder_fulfill(
                    output_template, length, self.output_motion_holder_seq, texts[i]
                )
            )
    ...
    def template_fulfill_pair(self, tasks, lengths_a, lengths_b, texts):
        inputs = []
        outputs = []
        modes = []
        ...
        for i in range(len(lengths_a)):
            ...
            output_template = random.choice(tasks[i]["output"])
            inputs.append(
                self.placeholder_fulfill_pair(
                    input_template, lengths_a[i], lengths_b[i], texts[i]
                )
            )
            outputs.append(
                self.placeholder_fulfill_pair(
                    output_template, lengths_a[i], lengths_b[i], texts[i]
                )
            )
            modes.append(tasks[i]["class"])
    ...
    def forward_dec_two_motion(
        ...
        self.tokenizer.padding_side = "right"
        with torch.no_grad():
            inputs, outputs, modes = self.template_fulfill_pair(
                tasks, lengths_source, lengths_target, texts
            )
            labels = []
            for i in range(len(inputs)):
                labels.append(
        ...
        labels_input_ids = inputs_tok.input_ids.to(motion_source.device)
        attention_mask = inputs_tok.attention_mask.to(motion_source.device)
        input_is_motion = labels_input_ids == self.motion_holder_id

        type_ids = torch.zeros_like(labels_input_ids).to(motion_source.device)
        input_type_ind = labels_input_ids >= self.text_vocab_size
        type_ids[input_type_ind] = 1
        eom_ignore = labels_input_ids == self.eom_id
        labels_input_ids[input_type_ind] -= self.text_vocab_size
        ...
        outputs.diff_loss = torch.tensor(0.0, device=motion_source.device)
        return outputs
    ...
    def create_sentinel_ids(self, mask_indices):
        ...
        sentinel_ids = np.where(
            sentinel_ids != 0,
            (len(self.tokenizer) - sentinel_ids - (self.m_codebook_size + 3)),
            0,
        )
