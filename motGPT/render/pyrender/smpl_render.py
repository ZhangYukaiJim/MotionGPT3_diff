...
class Renderer:
    ...
    def __init__(self, vertices, focal_length=5000, img_res=(224, 224), faces=None):
        ...
        self.floor_pose = np.array(
            [
                [1, 0, 0, 0],
                [0, np.cos(np.pi / 2), -np.sin(np.pi / 2), miny + 0.1],
    ...
    def __call__(self, vertices, camera_translation):
        ...
        cam_pose = np.array(self.camera_pose)
        cam_t = (
            camera_translation[0]
            if isinstance(camera_translation, list)
            else camera_translation
        )
        cam_pose[:3, 3] += cam_t.cpu().numpy()  # apply translation
        ...
        scene = pyrender.Scene(
            bg_color=(1.0, 1.0, 1.0, 0.8), ambient_light=(0.4, 0.4, 0.4)
        )
...
class SMPLRender:
    ...
    def init_renderer(self, res, smpl_param, is_headroot=False):
        ...
        for pose in poses:
            ...
            pred_rotmats.append(
                torch.from_numpy(pose.astype(np.float32)[None]).to(self.device)
            )
        ...
        pred_root = torch.tensor(
            smpl_param["pred_root"].reshape(-1, 3).astype(np.float32),
            device=self.device,
        )
        ...
        pred_root[:, 2] = -pred_root[:, 2]
        pred_root[:, 0] = 0
        pred_root[:, 1] = 0
        self.pred_camera_t = pred_root
    ...
    def render(self, index):
        renderImg = self.renderer(self.vertices[index, ...], self.pred_camera_t[index])
        return renderImg
