...
def create_logger(cfg, phase="train"):
    ...
    if cfg.TRAIN.RESUME and phase == "train":
        resume_path = Path(cfg.TRAIN.RESUME)
        if resume_path.is_file():
            final_output_dir = resume_path.parent.parent
        else:
            final_output_dir = resume_path
        cfg.NAME = final_output_dir.name
    else:
        cfg_name = cfg.NAME
...
def new_dir(cfg, phase, time_str, final_output_dir):
    ...
    if (
        ...
        and not os.path.exists(cfg.TRAIN.RESUME)
        and not cfg.DEBUG
